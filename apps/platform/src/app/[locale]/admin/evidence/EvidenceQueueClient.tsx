...
export function EvidenceQueueClient({ pendingEvidence, allEvidence }: Props) {
  ...
  async function handleAction(id: string, action: "verify" | "reject") {
    ...
    try {
      ...
      toast.success(action === "verify" ? t("toast.verified") : t("toast.rejected"));
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Unknown error";
      toast.error(t("toast.failed", { message: msg }));
    } finally {
      setLoadingIds((prev) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: t("stats.total"), value: allEvidence.length, alert: false, icon: FileText },
          { label: t("stats.verified"), value: verifiedCount, alert: false, icon: CheckCircle2 },
          { label: t("stats.rejected"), value: rejectedCount, alert: false, icon: XCircle },
        ].map((s) => (
      ...
      <Tabs defaultValue="pending">
        <TabsList>
          ...
          <TabsTrigger value="all">{t("tabs.allEvidence")}</TabsTrigger>
        </TabsList>
                    ...
                    <CardContent className="py-4 space-y-3 flex flex-col">
                      ...
                      {e.proxyMethod && (
                        <div className="rounded-lg bg-purple-50 border border-purple-200 px-3 py-2 text-xs text-purple-800">
                          <span className="font-medium">{t("proxyMethod")}</span> {e.proxyMethod}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t("table.indicator")}</TableHead>
