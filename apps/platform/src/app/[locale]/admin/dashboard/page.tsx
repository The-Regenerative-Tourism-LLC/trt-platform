  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          { label: t("stats.operators"), value: operatorCount },
          { label: t("stats.totalScores"), value: totalScores },
          { label: t("stats.published"), value: publishedScores },
          { label: t("stats.pendingEvidence"), value: pendingEvidence, alert: pendingEvidence > 0 },
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        >
          {t("actions.manageOperators")}
        </Link>
        <Link
          href="/admin/evidence"
          className="px-4 py-2 border rounded-full text-sm hover:bg-muted transition-colors"
        >
          {t("actions.evidenceQueue")}{pendingEvidence > 0 ? ` (${pendingEvidence})` : ""}
        </Link>
        <Link
          href="/admin/territories"
          className="px-4 py-2 border rounded-full text-sm hover:bg-muted transition-colors"
        >
          {t("actions.territories")}
        </Link>
        <Link
          href="/admin/scores"
          className="px-4 py-2 border rounded-full text-sm hover:bg-muted transition-colors"
        >
          {t("actions.scoreSnapshots")}
        </Link>
      </div>
      ...
      <div className="rounded-lg border border-border bg-card shadow-sm">
        <div className="p-4 border-b">
          <h2 className="font-semibold">{t("recentScores.title")}</h2>
        </div>
