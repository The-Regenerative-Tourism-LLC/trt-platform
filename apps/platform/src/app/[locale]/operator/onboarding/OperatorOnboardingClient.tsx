...
export function OperatorOnboardingClient() {
  ...
  const tSave = useTranslations("onboarding.navigation");
  const {
    stepId,
    data,
    updateField,
  ...
  useEffect(() => {
    if (onboardingData?.operator?.onboardingCompleted === true) {
      router.replace(withLocale("/operator/dashboard"));
    }
  }, [onboardingData, router, locale]);
  ...
  const handleSaveClose = async () => {
    try {
      ...
      router.push(withLocale("/operator/dashboard"));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!operator?.id || !territoryId) {
        toast.error(tCommon("submissionProfileIncomplete"));
        return;
      }
      ...
      if (!res.ok) {
        toast.error(json.error ?? tCommon("submissionFailed"));
        return;
      }
      ...
      toast.error(tCommon("submissionRequestFailed"));
      console.error(err);
    } finally {
      setSaving(false);
    }
  ...
  if (submissionSuccess) {
    return (
      <SubmissionSuccessScreen onGoToDashboard={() => router.push(withLocale("/operator/dashboard"))} />
    );
  }
  ...
  switch (stepId) {
    ...
    default:
      return (
        <StepShell
          {...shell}
          title={currentStep?.label ?? tCommon("unknownStep")}
