...
export default async function PublicGreenPassportPage({ params }: Props) {
  const { id } = await params;
  const t = await getTranslations("public.operators");
      ...
      <div className="max-w-4xl mx-auto px-4 pt-20 pb-12 space-y-8">
        ...
        <div className="rounded-lg border border-border bg-card p-6 md:p-8 space-y-6 shadow-sm">
          ...
          <div className="space-y-5">
            <PillarBar
              label={t("detail.p1.barLabel")}
              score={p1}
              weight={0.4}
              colorClass="bg-[hsl(var(--gps-footprint))]"
            />
            <PillarBar
              label={t("detail.p2.barLabel")}
              score={p2}
              weight={0.3}
              colorClass="bg-[hsl(var(--gps-local))]"
            />
            <PillarBar
              label={t("detail.p3.barLabel")}
              score={p3}
              weight={0.3}
          ...
          <div className="grid md:grid-cols-3 gap-4 pt-2">
            {[
              {
                label: t("detail.p1.label"),
                score: p1,
                description: t("detail.p1.description"),
                color: "border-border bg-secondary/50",
              },
              {
                label: t("detail.p2.label"),
                score: p2,
                description: t("detail.p2.description"),
                color: "border-border bg-secondary/50",
              },
              {
                label: t("detail.p3.label"),
                score: p3,
                description: t("detail.p3.description"),
                color: "border-border bg-secondary/50",
              },
            ].map((pillar) => (
              <div
          ...
          <div className="rounded-lg border border-border bg-card p-6 space-y-4 shadow-sm">
            ...
            {score.dps1 != null && (
              <div className="grid grid-cols-3 gap-3">
                {[
                  { label: t("detail.dps.rate"), value: Number(score.dps1) },
                  { label: t("detail.dps.consistency"), value: score.dps2 ? Number(score.dps2) : null },
                  { label: t("detail.dps.p3Accel"), value: score.dps3 ? Number(score.dps3) : null },
                ].map((d) => (
        ...
        {operator.territory?.compositeDpi != null && (
          <div className="rounded-lg border border-border bg-card p-6 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: t("detail.dpiMetrics.touristIntensity"), value: operator.territory.touristIntensity, weight: "35%" },
                { label: t("detail.dpiMetrics.ecologicalSensitivity"), value: operator.territory.ecologicalSensitivity, weight: "30%" },
                { label: t("detail.dpiMetrics.economicLeakage"), value: operator.territory.economicLeakageRate, weight: "20%" },
                { label: t("detail.dpiMetrics.regenPerformance"), value: operator.territory.regenerativePerformance, weight: "15%" },
              ].map((m) => (
        ...
        <div className="rounded-lg border border-border bg-muted/30 p-6 space-y-3 shadow-sm">
          ...
          <div className="grid sm:grid-cols-2 gap-2 text-xs text-muted-foreground">
            ...
            <p>
              <span className="font-medium text-foreground">{t("detail.auditComputed")}:</span>{" "}
            ...
            <p className="sm:col-span-2">
              <span className="font-medium text-foreground">{t("detail.auditSnapshotId")}:</span>{" "}
