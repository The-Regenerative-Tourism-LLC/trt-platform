...
export function TrackingScripts() {
  ...
  // Wait until consent is read from cookie before rendering any scripts
  if (!mounted) return null;

  return (
    <>
      {/* Klaviyo — marketing consent required */}
      {consent.marketing && (
        <Script
      ...
      {isProduction && (
        <>
          {/* Google Analytics — analytics consent required */}
          {consent.analytics && (
            <>
              <Script
          ...
          {/* Google AdSense — marketing consent required */}
          {consent.marketing && (
            <Script
